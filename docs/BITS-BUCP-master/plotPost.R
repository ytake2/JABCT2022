plotPost = function( paramSampleVec , credMass=0.95 , compVal=NULL ,
    ...
    source("docs/BITS-BUCP-master/HDIofMCMC.R")
    ...
    # Plot histogram.
    jpeg(paste0(plotname, ".jpg"), quality = 100)
    if ( is.null(breaks) ) {
    ...
    if ( !is.null( ROPE ) ) {
      ropeCol = "darkred"
       pcInROPE = ( sum( paramSampleVec > ROPE[1] & max(paramSampleVec))
                            / length( paramSampleVec ) )
       lines( c(ROPE[1],ROPE[1]) , c(0.96*ROPEtextHt,0) , lty="dotted" , lwd=2 ,
              col=ropeCol )
       lines( c(max(paramSampleVec),max(paramSampleVec)) , c(0.96*ROPEtextHt,0) , lty="dotted" , lwd=2 ,
              col=ropeCol)
